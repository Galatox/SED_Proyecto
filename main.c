...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, LED_ROJO_Pin|LED_VERDE_Pin|LED_AZUL_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin : LISTO_Pin */
  GPIO_InitStruct.Pin = LISTO_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(LISTO_GPIO_Port, &GPIO_InitStruct);

  /*Configure GPIO pins : LED_ROJO_Pin LED_VERDE_Pin LED_AZUL_Pin */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
 	if(huart->Instance == USART2){
 		...
 		if((!strcmp(buffer,"\r")) || (!strcmp(buffer,"\n"))){
 				...
 				uartBuffer[uartIndice] = buffer[0];
 				uartIndice++;
 				memset(buffer,0,1);
 				HAL_UART_Receive_IT(&huart2, (uint8_t*)buffer, sizeof(buffer));
 			}
